...
class PuzzleApp:
    ...
    def get_path(self, node):
        ...
        while node:
            ...
            if node.action:  # Add the action only if it's not the initial node
    ...
    def bfs_search(self):
        ...
        while frontier:
            ...
            if node.state == self.goal_tiles:
                ...
                explored_count = len(explored) + 1      # count the goal state as explored
            ...
            explored_count = len(explored) + 1          # increment the explored counter
    ...
    def astar_search(self, heuristic):
        ...
        goal_state = self.goal_tiles

        frontier = []
        # Calculate initial heuristic
        initial_heuristic = self.calculate_heuristic (start_node.state, heuristic)  
        heapq.heappush(frontier, (initial_heuristic, start_node)) # add start node to the frontier

        # Dictionary for visited states
        state_to_node = {tuple(map(tuple, start_node.state)): start_node}
        explored = set()
        explored_count = 0

        # Open the file for storing fn, gn, hn values
        with open('evaluation.out', 'w') as file:
            while frontier:
                # Pop the node with the lowest f(n)
                _, node = heapq.heappop(frontier)
                current_state_tuple = tuple(map(tuple, node.state)) # current state tuple for comparison

                # Skip already explored states
                if current_state_tuple in explored:
                    continue
                explored.add(current_state_tuple)   # add current state to explored set
                ...
                for neighbor, _ in self.get_neighbors(node):
                    ...
                    g_n = node.path_cost + 1
                    h_n = self.calculate_heuristic(neighbor.state, heuristic)
                    f_n = g_n + h_n # Total cost (f(n) = g(n) + h(n))
                    neighbor.fn = f_n
                    neighbor.gn = g_n
                    neighbor.hn = h_n

                    # Add the neighbor to the frontier if it's not explored yet or has a better f(n)
                    if neighbor_state_tuple not in explored:
                        if neighbor_state_tuple not in state_to_node or f_n < state_to_node[neighbor_state_tuple].fn:
                            heapq.heappush(frontier, (f_n, neighbor))       # add neighbor to frontier
                            state_to_node[neighbor_state_tuple] = neighbor  # update visited nodes
                            file.write(f"f(n)={f_n}, g(n)={g_n}, h(n)={h_n}\n")
    ...
    def manhattan_distance(self, state):
        ...
        for i in range(3):
            for j in range(3):
                ...
                tile = state[i][j]
                if tile != 0: 
                 # Get the correct row and column in the goal state
                    goal_row, goal_col = divmod(tile - 1, 3)
                    hn += abs(goal_row - i) + abs(goal_col - j)
        return hn
    ...
    def misplaced_tiles(self, state):
        ...
        for i in range(3):
            for j in range(3):
                ...
                if tile != 0:
                    goal_row, goal_col = divmod(tile - 1, 3)
                    # Count the tile if it's not in the correct position
    ...
    def adjacent_tiles(self, state):
        goal_state = self.goal_tiles
        current_adjacent_pairs = []
        goal_adjacent_pairs = []

         # Traverse through the puzzle state to gather adjacent pairs
        for i in range(3):
            for j in range(3):
                # Check vertical pairs
                if i < 2:  
                    # Add vertical adjacent pairs if within bounds
                    current_adjacent_pairs.append((state[i][j], state[i + 1][j]))
                    goal_adjacent_pairs.append((goal_state[i][j], goal_state[i + 1][j]))
                # Check horizontal pairs
                if j < 2:  
                    # Add horizontal adjacent pairs if within bounds
                    current_adjacent_pairs.append((state[i][j], state[i][j + 1]))
                    goal_adjacent_pairs.append((goal_state[i][j], goal_state[i][j + 1]))
 
        # Count non-matching pairs or number of mismatches between current and goal adjacent pairs
        non_adjacent_count = sum(1 for pair in current_adjacent_pairs if pair not in goal_adjacent_pairs)
