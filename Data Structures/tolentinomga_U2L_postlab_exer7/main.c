...
{	// function for percolating up recursively until it reaches the root
	if (index > 0)
...
{	// function for percolating up recursively until no percolation is needed
	// IF IT HAS TWO CHILDREN, FIND THE SMALLER CHILD
	if (right(index) < H->size+1)
...
int *heapSort(HEAP *H)
{
	if (H != NULL && isEmpty(H) == 0)
	{
		HEAP* heapCopy = createHeap(H->maxSize);						// create copy of the heap using createHeap function with same maxSize as H
		int *sortArray = (int *) malloc(sizeof(int) * (H->maxSize));	// create a new array with size equal to the maxSize of H
		int size = H->size;	// hold the size of the original heap

		
		int copyCounter = 1;
		while(copyCounter <= size)
		{	// copying all the elements from the original heap to the copy heap
			insert(heapCopy, H->heap[copyCounter]); 
			copyCounter++;
		}

		int sortCounter = size;
		while(sortCounter > 0)
		{	// deleting the contents of the heap and storing them in sortArray
			sortArray[sortCounter] = deleteM(heapCopy);
			sortCounter--;
		}
